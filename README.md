...
Please be aware that the judgement program has already made a time shift for -5 channels (roughly equal to the time difference between a photon incoming and its electronic pulse peak), in order to simplify the time signal detection problem.     
...
### Results
Our model has balanced the result generation speed and the training accuracy. Currently (2019.07) we have submited the best result in all the three evaluation tests. (Submissions later than Jun.02 will not be ranked on the leaderboard for the First and Final Contest, since they have been ended)
